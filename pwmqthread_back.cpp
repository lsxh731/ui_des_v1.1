...
#ifdef EMGDATA
    if(QavePosi.size() > 8000)
        QavePosi.pop_front();
    QavePosi.push_back(Yn1);
#endif
    if(SmapCount > int(8000/Ctlfreq))
    {
        /* 原版
...
//    OutV += v;
//    if(OutV >2.5 ) OutV=2.5;
//    if(OutV < -2.5) OutV=-2.5;
    if(v >2.5 ) v=2.5;
    if(v < -2.5) v=-2.5;
    tmp_v = EctlRatio*((v+10)/20)*4095/100;
    bf[7]=(tmp_v>>8)&0xff;
    bf[8]=tmp_v&0xff;
